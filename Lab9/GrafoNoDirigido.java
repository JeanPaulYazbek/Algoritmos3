...
public class GrafoNoDirigido<V, L> implements Grafo<V, L>
{
	...
	{	
		...
		try{
			numeroLados = Integer.parseInt(cuantosVerticesLados[1]);//numero de lados
		}catch(Exception e){
			return false;//si no es un entero formato erroneo
		}

		String id;//guarda ids que leamos
		V dato =null;//guarda datos que leamos
		double peso= 0.0;//guarda peso que leamos
		...
		L datoarista = null;	  //guarda dato de la arista leida
		double pesoarista = 0.0;//guarda peso de la arista leida
		...
		for(int k = 0; k<numeroLados; k++)
		{
			...
			try{
				idarista  = String.valueOf(k);
				pesoarista = Double.valueOf(vertice[2]);
				extremo1= vertice[0];
				extremo2= vertice[1];
			...
			agregarArista(idarista, datoarista, pesoarista, extremo1, extremo2);
	 ...
	 */
	public ArrayList<Arista<L>> incidentes(String id)
	{
		if(estaVertice(id))
		{
	...
	*/
	 ...
	 */
	public GrafoNoDirigido<V, L> clone()
	{
		GrafoNoDirigido<V, L> clon = new GrafoNoDirigido<V, L>();
		clon.grafo = deepCopy(grafo);
		return clon;
	}
