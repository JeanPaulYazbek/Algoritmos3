...
public class GrafoDirigido<V, L> implements Grafo<V, L>{
	...
	throws IOException
	{
		...
		for(int x = 1; x < numeroLinea + 1; x++){//saltamos lineas hasta llegar al caso
   			linea = Lector.readLine();
		}
		
		String[] cuantosVerticesLados = linea.split(" ");
		
		//Buscamos errores en la linea
		int numeroVertices;
		int numeroLados;
		if(cuantosVerticesLados.length != 2){//en caso de formato erroneo
			return false;
		}
		try{
			numeroVertices = Integer.parseInt(cuantosVerticesLados[0]);
			numeroLados = Integer.parseInt(cuantosVerticesLados[1]);
		}catch(Exception e){
			return false;//si no es un entero formato erroneo
		}

		if (numeroVertices == 0 && numeroLados == 0){
			return false;//se terminaron los casos de prueba
		}
		//AGREGAR LOS VERTICES
		String idVertice;
		V dato =  null;
		for(int k = 1; k<numeroVertices+1; k++){

			idVertice = String.valueOf(k);//el id del vertice sera el orden en que lo encontremos
		...
		for(int k = 0; k<numeroLados; k++){
			...
			agregarArco(linea, datoarco, 0.0, vertice[1], vertice[0]);

		}

		//agregamos una fuente fantasma

		/*agregarVertice("fuente Fantasma", dato, 0.0);

		for (Vertice<V> verticeFresco: digrafo.keySet()){

			if (gradoInterior(verticeFresco.getId()) == 0 && !(verticeFresco.getId().equals("sumidero Fantasma") ||
			verticeFresco.getId().equals("fuente Fantasma"))){//si es fuente parcial

				agregarArco("fuente Fantasma" + verticeFresco.getId(), datoarco, 0.0, "fuente Fantasma", verticeFresco.getId());
			}
		}

		//agregamos un sumidero fantasma

		agregarVertice("sumidero Fantasma", dato, 0.0);

		for (Vertice<V> verticeFresco: digrafo.keySet()){

			if (gradoExterior(verticeFresco.getId()) == 0 && !(verticeFresco.getId().equals("sumidero Fantasma")
			|| verticeFresco.getId().equals("fuente Fantasma"))){//si es sumidero parcial

				agregarArco( verticeFresco.getId() + "sumidero Fantasma" , datoarco, 0.0, verticeFresco.getId(), "sumidero Fantasma");
			}
		}
	 ...
	 */
	public ArrayList<Arco<L>> incidentes(String id){

		ArrayList<Arco<L>> lados = new ArrayList<Arco<L>>();
		if(estaVertice(id)){
			int i;
